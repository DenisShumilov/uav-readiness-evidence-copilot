...
**10** агентів · **7** скілів · **1** рантайм-гейт · **~90** тестів · **5** форматів експорту · **4** демо-набори · **2** мови
...
CLI (інструмент командного рядка) знаходить тільки чотири задокументовані типи входів:
BOM-подібний CSV, Markdown-інструкцію, CSV журналу перевірок і Markdown QA-нотатки.
Відсутній доказ лишається `locked`; непідтримані файли ігноруються та показуються в ingest summary
(короткому підсумку прочитаних файлів).
...
- `npm run demo:tdp` — набір `demo-tdp-supplier-package` (**49/100**, Blocked): TDP-style пакет постачальника з двома locked-твердженнями, одним partial cross-reference і виведеним конфліктом ревізій.
...
`npm run demo:selfaudit` наводить **той самий рушій** на ВЛАСНУ реальну документацію репозиторію (не синтетику): перевіряє, що кожна задокументована оцінка демо збігається з тим, що рушій реально обчислює, що «10 агентів / 7 скілів» — це справжні файли, що двомовні двійники існують, що згадані ассети на місці, і що release tag (мітка релізу) на live-site збігається з `README.md`. Чистий репозиторій дає **100/100 без жодного override** — а щойно док «дрейфне» (застаріла оцінка, відсутній двійник або застарілий release tag), рушій **перекриває** задокументоване твердження (`verified -> locked`) і **CI падає**. Це рушій виводить вердикт на вводі, який сам не писав.
