...
- **Тест-твердження оцінюються за власним результатом перевірки.** Перевірка з міткою `verified`, чий
  ...
  стає `partial`.
- **Суперечність між документами → `conflict`.** Заявлені ревізії порівнюються між документами
  (`packages/core/src/crossCheck.ts`); незгода **виводиться**, а не вписується вручну.
