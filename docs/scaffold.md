...
Запусти `npm run demo:gate`, щоб двічі стартувати реальний hook: чиста правка документації завершується
з кодом 0, а заблокована фікстура завершується з кодом 2 і показує DENY stderr (stderr — текст помилки
від процесу). Команда також перезаписує закомічений sample log реальним запуском.

Гейт навмисно fail-closed (у разі сумніву блокує): лише мала задокументована allowlist (список
дозволених винятків) для безпечних словосполучень і самоцитування safety-policy прибирається перед
перевіркою denylist (списку заборонених слів); усе інше й далі блокується.
...
[моделі доказів](evidence-model.md), категорійній [мапі стандартів](standards.md) та в живому демо,
де оцінка готовності лишається строгою (`44/100` на синтетичних даних) саме тому, що непідтверджені
твердження не роздуваються — вони блокуються (locked).
