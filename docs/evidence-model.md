...
Доказ ревізії може бути структурованим (structured — із чіткою формою): BOM CSV має колонку
`revision` у форматі `subject=value`, а QA-нотатки можуть містити рядок `Revision: subject=value`.
Рушій порівнює ці поля й виводить `conflict`, якщо документи не збігаються; старий текстовий формат
`rev:subject=value` лишається fallback (запасним шляхом).
